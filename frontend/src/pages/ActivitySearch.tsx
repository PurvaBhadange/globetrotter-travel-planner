import React, { useState } from "react";
import { Navbar } from "../components/layout/Navbar";
import { Footer } from "../components/layout/Footer";
import { ACTIVITIES } from "../data/mockTravelData";
import { Search, Plus, Clock, Star, MapPin } from "lucide-react";
import { useNavigate } from "react-router-dom";

export const ActivitySearch: React.FC = () => {
  const [query, setQuery] = useState("");
  const [categoryFilter, setCategoryFilter] = useState("all");
  const navigate = useNavigate();

  const filtered = ACTIVITIES.filter((act) => {
    const matchesSearch =
      act.name.toLowerCase().includes(query.toLowerCase()) ||
      act.cityName.toLowerCase().includes(query.toLowerCase());
    const matchesCat = categoryFilter === "all" || act.category === categoryFilter;
    return matchesSearch && matchesCat;
  });

  return (
    <div className="min-h-screen bg-[#FFFDF5] text-black font-body">
      <Navbar />

      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        <div className="text-center mb-10">
          <span className="inline-block px-3 py-1 bg-[#00E5FF] text-black font-black text-xs uppercase tracking-widest border-2 border-black shadow-[2px_2px_0px_0px_#000] mb-3 rotate-[1deg]">
            EXPERIENCE CATALOG
          </span>
          <h1 className="font-black text-4xl sm:text-6xl text-black uppercase tracking-tighter leading-none mb-2">
            DISCOVER THINGS TO DO
          </h1>
          <p className="text-xs sm:text-sm font-bold text-black/80 max-w-lg mx-auto uppercase tracking-wide">
            Explore sightseeing, food tours, outdoor adventure, and cultural activities around the world.
          </p>
        </div>

        {/* Search & Filter Chips */}
        <div className="max-w-3xl mx-auto mb-12">
          <div className="relative mb-4">
            <Search className="w-5 h-5 text-black stroke-[3px] absolute left-4 top-4" />
            <input
              type="text"
              value={query}
              onChange={(e) => setQuery(e.target.value)}
              placeholder="SEARCH EIFFEL TOWER, LOUVRE, TORII HIKE, BAGA BOAT..."
              className="w-full bg-white border-4 border-black text-black font-bold text-sm placeholder-black/50 uppercase pl-12 pr-4 py-3.5 focus:outline-none focus:bg-[#FFD93D] shadow-[6px_6px_0px_0px_#000]"
            />
          </div>

          <div className="flex items-center justify-center flex-wrap gap-2">
            {["all", "sightseeing", "food", "adventure", "culture", "nature"].map((cat) => (
              <button
                key={cat}
                onClick={() => setCategoryFilter(cat)}
                className={`px-4 py-2 text-xs font-black uppercase tracking-wider border-2 border-black cursor-pointer transition-all ${
                  categoryFilter === cat
                    ? "bg-[#FF6B6B] text-white shadow-[2px_2px_0px_0px_#000]"
                    : "bg-white text-black hover:bg-[#FFD93D]"
                }`}
              >
                {cat}
              </button>
            ))}
          </div>
        </div>

        {/* Activities List */}
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {filtered.map((act) => (
            <div
              key={act.id}
              className="bg-white border-4 border-black shadow-[8px_8px_0px_0px_#000] card-neo-lift flex flex-col justify-between"
            >
              <div className="relative h-48 w-full border-b-4 border-black overflow-hidden">
                <img src={act.imageUrl} alt={act.name} className="w-full h-full object-cover" />
                <div className="absolute top-3 left-3 px-2.5 py-1 bg-[#FFD93D] text-black text-xs font-black uppercase border-2 border-black shadow-[2px_2px_0px_0px_#000]">
                  {act.category}
                </div>
                <div className="absolute top-3 right-3 px-2.5 py-1 bg-[#00E676] text-black text-xs font-black border-2 border-black shadow-[2px_2px_0px_0px_#000] flex items-center gap-1">
                  <Star className="w-3.5 h-3.5 fill-black text-black" />
                  <span>{act.rating}</span>
                </div>
              </div>

              <div className="p-5 flex-1 flex flex-col justify-between">
                <div>
                  <h3 className="font-black text-xl text-black uppercase tracking-tight mb-2">
                    {act.name}
                  </h3>
                  <div className="flex items-center gap-3 text-xs font-bold text-black/70 uppercase mb-3">
                    <span className="flex items-center gap-1 bg-[#FFFDF5] px-2 py-0.5 border border-black">
                      <MapPin className="w-3.5 h-3.5 text-[#FF3D00] stroke-[3px]" />
                      {act.cityName}
                    </span>
                    <span className="flex items-center gap-1 bg-[#FFFDF5] px-2 py-0.5 border border-black">
                      <Clock className="w-3.5 h-3.5 text-black stroke-[3px]" />
                      {act.durationMinutes} MINS
                    </span>
                  </div>
                  <p className="text-xs font-bold text-black/80 line-clamp-2 leading-snug uppercase mb-4">
                    {act.description}
                  </p>
                </div>

                <div className="flex items-center justify-between pt-3 border-t-3 border-black text-xs font-black">
                  <span className="bg-[#00E676] text-black px-2.5 py-1 border-2 border-black">
                    {act.cost > 0 ? `${act.currency} ${act.cost}` : "FREE ENTRY"}
                  </span>
                  <button
                    onClick={() => navigate("/trips/new")}
                    className="px-4 py-2 bg-[#FFD93D] text-black font-black text-xs uppercase border-2 border-black shadow-[3px_3px_0px_0px_#000] btn-neo-push hover:bg-[#FFC107] cursor-pointer flex items-center gap-1"
                  >
                    <Plus className="w-3.5 h-3.5 stroke-[3px]" />
                    <span>ADD TO TRIP</span>
                  </button>
                </div>
              </div>
            </div>
          ))}
        </div>
      </main>

      <Footer />
    </div>
  );
};

export default ActivitySearch;
